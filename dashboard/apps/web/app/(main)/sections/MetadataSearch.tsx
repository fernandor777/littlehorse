...
export function MetadataSearch() {
  ...
  const fetchData = async (type: string, paginate = false, useLimit = true) => {
    ...
    if (type === "wfSpec") {
      bookmark = wfSpecBookmark;
    }
    if (type === "taskDef") {
      bookmark = taskDefBookmark;
    }
    if (type === "userTaskDef") {
      bookmark = userTaskDefBookmark;
    }
    if (type === "externalEventDef") {
      bookmark = externalEventDefBookmark;
    }
    const filters: any = { limit: useLimit ? limit : allLimit };
    if (prefix?.trim()) {
      filters.prefix = prefix.trim().toLocaleLowerCase();
    }
    if (paginate && bookmark) {
      filters.bookmark = bookmark;
    }
    if (paginate && !bookmark) {
      return { status: "done" };
    }
  ...
  const getData = async () => {
    ...
    if (metadataType === "wfSpec") {
      setWfSpecBookmark(bookmark);
    }
    if (metadataType === "taskDef") {
      setTaskDefBookmark(bookmark);
    }
    if (metadataType === "userTaskDef") {
      setUserTaskDefBookmark(bookmark);
    }
    if (metadataType === "externalEventDef") {
      setExternalEventDefBookmark(bookmark);
    }
    setMetadataResults(
      results.map((v: Result) => ({
        ...v,
  ...
  const getMData = async () => {
    ...
    if (wfSpecs !== undefined && wfSpecs.results !== undefined) {
      ...
      setMetadataResults((_) =>
        wfSpecs.results.map((v: any) => ({ ...v, type: "WfSpec" }))
      );
    }
    ...
    <section>
      <div className="between">
        ...
        <div className="btns btns-right">
          ...
          <Button
            active={metadataType === "wfSpec"}
            onClick={() => {
              setMetadataType("wfSpec");
            }}
          >
            WfSpec
          </Button>
          <Button
            active={metadataType === "taskDef"}
            onClick={() => {
              setMetadataType("taskDef");
            }}
          >
            TaskDef
          </Button>
          <Button
            active={metadataType === "userTaskDef"}
            onClick={() => {
              setMetadataType("userTaskDef");
            }}
          >
            UserTaskDef
          </Button>
          <Button
            active={metadataType === "externalEventDef"}
            onClick={() => {
              setMetadataType("externalEventDef");
            }}
          >
            ExternalEventDef
          </Button>
      ...
      <div className="end">
        <div className="btns btns-right">
          ...
          <LoadMoreButton
            disabled={
              !externalEventDefBookmark && !wfSpecBookmark && !taskDefBookmark
            }
            loading={isLoading}
            onClick={loadMMore}
          >
